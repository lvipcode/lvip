...
DROP TRIGGER IF EXISTS trigger_redemption_codes_batch_stats ON redemption_codes;
DROP FUNCTION IF EXISTS update_orders_updated_at();
DROP FUNCTION IF EXISTS update_batch_stats();
DROP FUNCTION IF EXISTS generate_redemption_codes(VARCHAR, INTEGER, INTEGER, TEXT, VARCHAR);
...
-- 完成提示
SELECT '管理后台表结构创建完成！现在可以正常使用管理功能了。' as message;